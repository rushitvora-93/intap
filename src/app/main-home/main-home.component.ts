...
export class MainHomeComponent implements OnInit {
  ...
  delete(id) {
    ...
    if (confirm("Are you sure you want to delete?")) {
      ...
      this.authService.postData('updateUserSocial/' + id).subscribe(response => {
  ...
  directoff() {
    ...
    if (this.caption == 'Direct Off') {
      ...
      this.authService.postData('updateDirect/' + this.userName, body.toString()).subscribe(response => {
        if (response.data) {
  ...
  getData() {
    this.authService.setLoader(true);
    this.authService.getData('getUserSequence/' + this.userId).subscribe(response => {
      this.authService.setLoader(false);
      
      if (response.user) {
        const userData = response.user;
        this.fullName = userData[0].name;
        this.countTapin = userData[0].counter;
        if (userData[0].bio !== 'null') {
            this.desc = userData[0].bio;
          }
        if (userData[0].direct != null) {
          if(userData[0].direct === 'Direct On') {
            this.seen = false;
          } else {
            this.seen = true;
          }
          this.caption =  userData[0].direct;
        }
        }
      const data = [];
      if (response.data) {
  ...
  saveSocialSequence() {
    ...
    this.authService.setLoader(true);
    this.authService.postData('socialUserSequence/' + this.userId, body.toString()).subscribe(response => {
      this.authService.setLoader(false);
      if (response.status === 200) {
        this.getData();
        this.isDisabled = false;
  ...
  onClickWallet() {
    ...
    this.authService.setLoader(true);
    this.authService.getData('pkGenerateQ/' + this.userId).subscribe(response => {
      this.authService.setLoader(false);
